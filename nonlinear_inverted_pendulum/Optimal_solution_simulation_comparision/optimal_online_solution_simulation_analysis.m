...
global M; global m; global g; global L; global weights_opt;
...
weights_opt = [418.2181 174.1981 25.7084 73.7437 18.3557 5.4250 16.0159 2.8521 7.5543 10.4065]';
...
weights_opt = [0;2*K(1)/g0(2); 0; 0; K(2)/g0(2); 0; 0; 0; 2*K(3)/g0(4); K(4)/g0(4)]; % 14x1
Fsamples = 400;
T = 0.05;
uu = 0;
t_span = [0, Fsamples*T];
[t1, x1] = ode45("irl_2ode_nonlinear_opt", t_span, x0);
...
% plot(t1,x1(:, 1:4), "linewidth", 1.5)
% grid on
% xlabel('Time (s)');
% ylabel('rad');
% legendEntries = {'$\\\theta_{w0}$', '$\dot{\theta}_{w0}$', '$\\x_{IRL}$','$\dot{x}_{IRL}$'};
