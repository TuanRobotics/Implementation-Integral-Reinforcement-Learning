...
%% Control system using u0
global w1; global ww1;
w1 = [0 2 0]';
ww1 = [w1];
% Initial conditions [theta, theta_dot]
initial_conditions = [pi/6, 0,0];  % Initial angle (45 degrees) and angular velocity (0)
% xx1 = [initial_conditions];
j = 0;
nop = 3;
Fsamples = 300;
T = 0.03;
uu = 0;
t_span = [0, Fsamples*T];
[t1, xx1] = ode45(@(t, x) pendulum_dynamics_01(t, x, m, g, l, B), t_span, initial_conditions);
...
hold on 
plot(t1, xx1(:,1), "linewidth", 1.5)
grid on
legendEntries = {'с обучением, x_1(t)', 'без обучения, x_1(t)'};
legend('с обучением, x_1(t)', 'без обучения, x_1(t)','FontSize', 10);
...
hold on
plot(t1, xx1(:,2), "linewidth", 1.5)
grid on
% legendEntries = {'c обучением, x_2(t)', 'без обучения, $\x_2(t)$' };
legend('с обучением, x_2(t)', 'без обучения, x_2(t)','FontSize', 10);
...
grid on
xlabel('Время (с)');
title('Сходимости веса W при обучении', 'Interpreter','latex')
...
hold on
plot(t1, xx1(:,3), "linewidth", 1.5)
grid on
xlabel('Время (с)');
title('$J(x,u)$','Interpreter','latex');
legendEntries = {'J(x,u) с обучением', 'J(x,u), без обучения'};
legend(legendEntries, 'interpreter', 'latex','FontSize', 10);
figure(4), hold off;
...
function dxdt = pendulum_dynamics_01(t, x, m, g, l, B)
global u1; global Q; global R; global w1;
